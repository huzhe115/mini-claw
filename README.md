...
```
WebChat(网页) ┐
              ├─→ FastAPI 网关(认证/会话路由/SSE 流式桥接) ─→ Agent 大脑
CLI(命令行)   ┘        │                    (ReAct 循环 + 3 个工具)
                       ▼
                 PostgreSQL(会话/消息/原始对话,重启不丢)
```
...
- **Agent**:mini-claude 的核心循环迁移而来——模型要调工具时,后端替它执行
  (bash / read_file / write_file),结果回填继续思考;每步动作以事件形式流式推给前端
- **落库**:sessions(会话 + llm_messages JSONB)和 messages(展示消息 + meta)
  两张表;每次 Agent 运行的模型、token 用量、耗时记进 meta
...
1. Redis:任务状态、限流
2. 记忆系统(从 mini-claude 迁移 MemoryManager)+ cron 定时主动发消息
3. RabbitMQ 后台任务队列
4. 工具扩充:edit_file / glob / grep / todo_write
