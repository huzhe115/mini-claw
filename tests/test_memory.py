...
def test_extract_stores_persistent_only(mem, monkeypatch):
    candidates = [
        {"name": "用户叫小胡", "type": "user", "description": "名字",
         "body": "用户叫小胡。", "scope": "persistent"},
        {"name": "临时任务", "type": "project", "description": "这次要做的",
         "body": "本次会话的任务", "scope": "current_task"},
    ]
    monkeypatch.setattr("app.agent.memory.llm",
                        FakeCompleteLLM([json.dumps(candidates, ensure_ascii=False)]))
...
def test_should_store_filters(mem):
    good = {"name": "a", "type": "user", "description": "d", "body": "b",
            "scope": "persistent"}
...
def test_consolidate_merges(mem, monkeypatch):
    ...
    monkeypatch.setattr("app.agent.memory.llm",
                        FakeCompleteLLM([json.dumps(merged, ensure_ascii=False)]))
    mem.consolidate()
